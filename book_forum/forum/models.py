...
class Book(models.Model):
    ...
    status = models.CharField(max_length = 2, choices = Status.choices, default = Status.ONGOING, verbose_name="Статус")
    ...
    cover = models.ImageField(upload_to=covers_file_name, verbose_name="Обкладинка", default='default/default_cover.jpg')
...
class Book_comment(models.Model):
    ...
    book = models.ForeignKey(Book, on_delete=models.CASCADE, verbose_name="Книга", null=True, related_name='book_comments')
...
class Discussion(models.Model):
    ...
    search_title = models.CharField(max_length = 100)
    description = models.TextField(max_length = 2000, verbose_name="Опис")
    status = models.CharField(max_length = 2, choices = Status.choices, default = Status.OPENED, verbose_name="Статус")
    created_at = models.DateTimeField(auto_now_add = True, verbose_name="Створено")
    updated_at = models.DateTimeField(auto_now = True, verbose_name="Оновлено")
...
class Discussion_comment(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    discussion = models.ForeignKey(Discussion, on_delete=models.CASCADE, verbose_name="Обговорення", null=True, related_name='discussion_comments')
    user = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, verbose_name="Користувач")

    answer = models.ForeignKey("self", on_delete = models.CASCADE, null=True, blank=True, verbose_name="Відповідь на",  related_name='discussion_answer')
    content = models.TextField(max_length = 2000, verbose_name="Зміст коментаря")
    created_at = models.DateTimeField(auto_now_add = True, verbose_name="Дата")
